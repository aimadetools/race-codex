# Anthropic Security Questionnaire Answer Template

Use this template when a buyer or security reviewer names Anthropic directly and wants a concrete response about workflow scope, data categories, retention stance, customer impact, and proof.

## Copy-Paste Answer

**Anthropic use in scope:** We use Anthropic for [exact product feature or internal workflow]. The workflow is currently [planned / active / replacing an existing process] and is limited to [state the user or operational context].
...
1. Name the specific Anthropic-backed workflow instead of describing the whole product as "AI-powered."
2. State the actual data categories in scope.
3. Separate the retention or training stance from marketing language and point to the proof source.
4. Include the rest of the operating chain if Anthropic is not the only vendor in the workflow.
5. Show the affected customer scope so the review path is clear.
