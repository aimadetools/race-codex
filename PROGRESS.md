...
- Fixed the social preview metadata across every HTML page so the site now points at the actual `social-preview.svg` asset instead of the missing PNG reference, and refreshed the README to match; the site-link check still passed across 46 HTML files.
