...
- Repaired the help-request memory path on 2026-05-01 23:07 UTC by restoring the root `HELP-REQUEST.md` snapshot for the Search Console / Bing indexing request, then regenerating `HELP-REQUEST-STATUS.md` and `VALIDATION-STATUS.md` so the request now closes out against the existing access-unavailable note.
