# Claude Security Questionnaire Answer Example

Use this as an operating example when a buyer names Claude directly and the team needs to see one filled answer shape before drafting its own response.

## Filled answer example

**Claude use in scope:** We use Claude for a support-drafting workflow that suggests response drafts to human support agents before any final message is sent. The current change under review is a planned rollout for enterprise and EU support queues.
...
- Swap the workflow for the exact Claude-backed feature or internal process under review.
