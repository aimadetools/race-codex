...
- `scripts/send-partner-outreach.mjs` dry-runs or sends the first ready partner-program messages and the due partner follow-up pass through Resend using the same researched direct-email routes as advisor batch 02; follow-up mode also accepts `--organization` and `--follow-up-asset kit-preview` for cohort-specific CTA tests.
