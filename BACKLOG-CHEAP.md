...
- 2026-05-06: Added explicit watch coverage for `founder-follow-up-teardown` and `advisor-follow-up-teardown`, shipped a shared source-tag registry plus `npm run check:source-tag-coverage`, and reran the 04:10 UTC maintenance/artifact sync so outreach-driven teardown attribution cannot drift silently.
- 2026-05-06: Ran the 04:13 UTC validation maintenance pass, refreshed the generated help/inbox/generator/partner/validation snapshots, and preserved the no-reply checkpoint with the live queue still at 0 real submissions, 0 replies, and 0 interviews.
- 2026-05-06: Preserved the full forwarded intake shape in the webhook archive path, added `npm run check:contact-webhook-record` to keep source/role/partner/self-audit fields from drifting out of the backup record, and reran the 04:16 UTC maintenance/artifact sync with the live queue still at 0 real submissions, 0 replies, and 0 interviews.
- 2026-05-06: Fixed the `free-teardown.html` to `audit-request.html` escalation link so typed form values stay attached when a visitor switches from the short teardown form to the full intake, and added `npm run check:free-teardown-handoff` to keep that handoff from regressing.
- 2026-05-06: Ran the 04:19 to 04:25 UTC maintenance/artifact refresh passes, kept the live queue at 20 active outbound rows with 0 real submissions, 0 replies, and 0 interviews, and promoted inbox ownership-signal plus partner-goal counts into the generated status files so the first real teardown or partner request can be qualified directly from the repo snapshots.
