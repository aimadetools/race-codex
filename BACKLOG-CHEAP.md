...
- On or after `2026-05-04` UTC, if the five partner-program sends still show zero replies, run `npm run send:partner-follow-up -- --send` and log any resulting reply or terminal status change in `consultant-partner-outreach-tracker.csv`; if testing the founder-safe handoff framing, send the chosen cohort with `--organization "<names>" --follow-up-asset client-handoff`, and if testing the concrete deliverable framing, use `--follow-up-asset kit-preview` so the tracker notes preserve who received each variant.
